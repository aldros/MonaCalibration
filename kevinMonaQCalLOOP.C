...
void kevinMonaQCalLOOP(bool graphing = true) {
	...
	for (int y=0; y<nVert; y++){
		for (int z=0; z<nSides; z++){
	    	...
	    	// declare hists
		    TH1F *h1 = new TH1F(Form("raw[%d][%d][%d]",x,y,z),"Raw charge;Channel;Counts",300,0,4096);
		    TH1F *h2 = new TH1F(Form("cal[%d][%d][%d]",x,y,z),"Calibrated charge;Energy (MeV);Counts",100,0,80);
			...
			std::cout << "Calibrated peak: " << xPeaks[0] << "MeV in " << realTime <<  "s " << std::endl;
